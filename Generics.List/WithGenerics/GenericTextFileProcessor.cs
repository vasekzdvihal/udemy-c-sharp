...
namespace Generics.List.WithGenerics
{
    public class GenericTextFileProcessor
    {
        public static List<T> LoadFromTextFile<T>(string filePath) where T : class, new()
        {
            ...
            foreach (var row in lines)
            {
                ...
                // Splits the row into individual columns. Now the index of this row matches the index of the header so the FirstName column header lines up with the FirstName value in this row
                var vals = row.Split(',');
                
                // Loops through each header entry s owe can compare that against the list of columns from reflection. Once we get the matching column we can do the "SetValue" method to set the column value for our entry variable to the val item at the same index as this particular header.
        ...
        public static void SaveToTextFile<T>(List<T> data, string filePath) where T : class, new()
        {
            ...
            {
                throw new ArgumentNullException("data", "You must populate the data parameter with at least one row");
            }
